...
export default function ShoapPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // ✅ category name DB me "shoap" save karo
        const res = await fetch("/api/products?category=shoap", {
          cache: "no-store",
          ...
          <div className={styles.page}>
            <div className={styles.header}>
              <div className={styles.heading}>
                <h1 className={styles.title}>Shoap</h1>
                <p className={styles.subText}>{urlQ ? `Results for "${urlQ}"` : "Browse shoap products"}</p>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <p className={styles.empty}>No shoap products found.</p>
            ) : filtered.length === 0 ? (
